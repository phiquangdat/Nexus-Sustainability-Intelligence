...
const Navbar = () => {
  ...
  return (
    <nav className="bg-white/90 backdrop-blur-sm shadow-xl border-b border-neutral-200/50 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-20">
          ...
          <div className="flex items-center">
            ...
            >
              <div className="w-12 h-12 bg-gradient-to-r from-primary-500 to-secondary-600 rounded-xl flex items-center justify-center shadow-lg group-hover:shadow-glow transition-all duration-300 transform group-hover:scale-105">
                ...
                {item.children ? (
                  <>
                    ...
                    {activeDropdown === item.id && (
                      <div className="absolute top-full left-0 mt-2 w-64 bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl border border-neutral-200/50 z-50 overflow-hidden animate-slide-down">
                        <div className="py-2">
                          {item.children.map((child) => (
                            <Link
                              ...
                              to={child.href}
                              onClick={handleLinkClick}
                              className={`flex items-center space-x-3 px-6 py-3 text-sm font-medium transition-all duration-300 hover:transform hover:scale-105 ${
                                isActive(child.href)
          ...
          <div className="md:hidden flex items-center">
            <button
              ...
              className="inline-flex items-center justify-center p-2 rounded-md text-neutral-700 hover:text-primary-600 hover:bg-neutral-100 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-primary-500 transition-all duration-300"
            >
                ...
                <div key={item.id}>
                  {item.children ? (
                        ...
                        <div className="pl-6 space-y-1 animate-slide-down">
                          {item.children.map((child) => (
                            <Link
                              ...
                              to={child.href}
                              onClick={handleLinkClick}
                              className={`flex items-center space-x-2 px-3 py-2 rounded-md text-sm transition-all duration-300 hover:transform hover:scale-105 ${
                                isActive(child.href)
                                  ? "text-primary-600 bg-primary-50"
                                  : "text-neutral-700 hover:text-primary-600 hover:bg-neutral-50"
                              }`}
                    ...
                    <Link
                      to={item.href}
                      onClick={handleLinkClick}
                      className={`flex items-center space-x-2 px-3 py-2 rounded-md text-base font-medium transition-all duration-300 hover:transform hover:scale-105 ${
                        isActive(item.href)
                          ? "text-primary-600 bg-primary-50"
                          : "text-neutral-700 hover:text-primary-600 hover:bg-neutral-50"
                      }`}
